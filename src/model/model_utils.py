...
class RemoteOpenAIChatAgent:
    """Small OpenAI-compatible chat-completions client used for API-hosted models."""

    def __init__(self, args):
        self.model = args.api_model or args.model
        self.api_base = args.api_base.rstrip("/")
        self.api_key = args.api_key or "EMPTY"
    ...
    def generate(self, messages):
        ...
        raise RuntimeError(f"Remote model request failed after {self.retries + 1} attempts: {last_error}")


def _normalize_chat_messages(message):
...
def engine(messages, agent, num_agents=1, stop_sequences=None):
    if isinstance(agent, RemoteOpenAIChatAgent):
        with ThreadPoolExecutor(max_workers=min(len(messages), max(1, num_agents))) as executor:
            return list(executor.map(lambda message: agent.generate(_normalize_chat_messages(message)), messages))
...
def get_agents(args, peft_path=None):
    ...
    if args.api_base:
        agent = RemoteOpenAIChatAgent(args)
    ...
    # update pad token
    if hasattr(agent, "tokenizer") and agent.tokenizer.pad_token is None :
