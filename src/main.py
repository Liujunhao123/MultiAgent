...
def get_args():
    ...
    parser.add_argument('--toy_data', action='store_true', help='Use built-in small datasets for local smoke-test experiments.')
    ...
    parser.add_argument('--api_base', type=str, default='', help='OpenAI-compatible API base URL, e.g. http://host:port/v1')
    parser.add_argument('--api_key', type=str, default='', help='API key for OpenAI-compatible remote inference')
    parser.add_argument('--api_model', type=str, default='', help='Remote model name. Defaults to --model when omitted.')
    ...
    parser.add_argument('--aggregation', type=str, default='vote', choices=['vote','judge'], help='Final-answer aggregation strategy.')
...
def get_judge_message(args, sample, responses, suffix=None):
    msg = (
        "You are a careful final judge for a multi-agent reasoning task.\n"
        "Review the question and all agent responses. Prefer answers with correct reasoning, "
        "not merely the most frequent answer. If agents disagree, resolve the disagreement step by step.\n\n"
        f"Question:\n{sample}\n\n"
        "Agent responses:"
    )
    for name, response in responses.items():
        msg += f"\n\n{name}:\n{response}"
    msg += "\n\nNow provide the single best final answer."
    if suffix is not None:
        msg += suffix
    return {'role': 'user', 'content': msg}


def aggregate_and_evaluate(args, responses, answer, evaluate, sample, agent, suffix):
    agent_final_resps, agent_debate_resps, agent_is_corr = evaluate(responses, answer)
    if args.aggregation == 'vote':
        return agent_final_resps, agent_debate_resps, agent_is_corr, None

    judge_message = get_judge_message(args, sample, responses, suffix=suffix)
    judge_response = engine([judge_message], agent, 1)[0]
    judge_final_resps, judge_debate_resps, judge_is_corr = evaluate({'Judge': judge_response}, answer)
    metadata = {
        'agent_vote_final_answers': agent_final_resps,
        'agent_vote_answer': agent_debate_resps,
        'agent_vote_iscorr': agent_is_corr,
        'judge_response': judge_response,
    }
    return judge_final_resps, judge_debate_resps, judge_is_corr, metadata
...
def main(args):
    ...
    if args.toy_data : fname += '_TOY'
    if args.aggregation != 'vote' : fname += f'_{args.aggregation.upper()}'
    ...
    for i, (x, y) in tqdm(enumerate(zip(test_X, test_Y)), total=len(test_X)):
        ...
        if args.centralized :
            ...
            final_resps, debate_resps, is_corr, aggregation_metadata = aggregate_and_evaluate(args, central_agent_response, y, evaluate, x, agent, SUFFIX)
        else :
            final_resps, debate_resps, is_corr, aggregation_metadata = aggregate_and_evaluate(args, agent_responses, y, evaluate, x, agent, SUFFIX)
        ...
        if aggregation_metadata is not None:
            round_data['aggregation'] = aggregation_metadata
    ...
    with open(os.path.join(args.out_dir, 'logs.tsv'), 'a') as f :
