...
def _extract_braced_final_answer(response):
    matches = re.findall(r"\{\s*final answer\s*:\s*([^{}]+)\}", response, flags=re.IGNORECASE)
    if matches:
...
def evaluate_arithmetics(responses, answer):
    ...
    for _, response in responses.items():
        ...
        try:
            pred = _extract_braced_final_answer(response)
            if pred is None:
...
def evaluate_gen(responses, answer):
    if ROUGE is None:
        raise ImportError("rouge_score is required for cnn_daily evaluation. Install rouge-score to use this task.")
